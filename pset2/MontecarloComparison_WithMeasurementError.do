...
forvalues i=1/200{
	...
	*Use analytical relations to construct inputs and revenue (in the process we also create some unobserved variables but we will not use those for estimation)
	...
	forvalues t=0/9{
		...
		*Include measurement error (note that it is not affecting decisions)
		replace K_`tt'=K_`tt'*rnormal(0,0.1)
	}
...
foreach param in b0 bk bl Rho{
	...
	graph export "D:\DDC\PhD_IO\pset2\MS_`param'_MeasurementError.png", as(png) name("Graph") replace
